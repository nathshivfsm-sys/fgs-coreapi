...
#           notification-service, file-service, inventory-service, consumer-service, nginx

set -euo pipefail

COMPOSE_SERVICE="${1:?compose service (redis, rabbitmq, setup-service, audit-service, user-service, bff-service, notification-service, file-service, inventory-service, consumer-service, nginx)}"
...
upsert_env FGS_AUDIT_IMAGE "${REGISTRY}/${ECR_REPO}:audit-${CHANNEL}"
