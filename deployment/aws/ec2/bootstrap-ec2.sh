...
echo "  2. Ensure glo.GloCredential has Global:DATABASE (FgsUser, FgsAudit, FgsNotification, FgsSetup, FgsInventory, FgsAsset, …),"
...
echo "     redis → rabbitmq → setup → audit → user → bff → notification → file → inventory → asset → consumer → nginx"
...
echo "     sudo $FGS_DIR/deploy-service.sh inventory-service dev"
echo "     sudo $FGS_DIR/deploy-service.sh asset-service dev"
