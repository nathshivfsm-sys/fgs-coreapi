...
| Images | Built locally from Dockerfiles | Pulled from ECR (`setup-dev`, `user-dev`, `audit-dev`, `inventory-dev`, …) |
...
```bash
# On EC2 (as root), after copying updated files from the repo:
...
```bash
sudo /opt/fgs/deploy-service.sh redis dev
sudo /opt/fgs/deploy-service.sh rabbitmq dev
sudo /opt/fgs/deploy-service.sh setup-service dev
sudo /opt/fgs/deploy-service.sh audit-service dev
