...
public sealed class GetAttachmentStreamQueryHandler(
    IUnitOfWork unitOfWork,
    ITenantContextAccessor tenantContextAccessor,
    IFileStorageService fileStorageService)
    : IRequestHandler<GetAttachmentStreamQuery, ApiResponse<AttachmentStreamModel>>
{
    public async Task<ApiResponse<AttachmentStreamModel>> Handle(
        GetAttachmentStreamQuery request,
        CancellationToken cancellationToken)
    {
        ...
        var result = await fileStorageService.OpenReadAsync(
            new StorageObjectRef(file.BucketName, file.ObjectKey),
            request.Range,
            cancellationToken);
