...
RUN --mount=type=cache,target=/root/.nuget/packages \
    dotnet build Fgs.Asset.API.csproj -c Release --no-restore
...
ENTRYPOINT ["dotnet", "run", "--no-build", "--no-launch-profile", "--project", "Fgs.Asset.API.csproj", "--configuration", "Release", "--urls", "http://+:5015"]
