...
RUN --mount=type=cache,target=/root/.nuget/packages \
    dotnet build Fgs.Crm.API.csproj -c Release --no-restore
...
ENTRYPOINT ["dotnet", "run", "--no-build", "--no-launch-profile", "--project", "Fgs.Crm.API.csproj", "--configuration", "Release", "--urls", "http://+:5009"]
