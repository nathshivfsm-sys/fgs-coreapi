...
COPY src/BffService/Fgs.Bff.API/Fgs.Bff.API.csproj src/BffService/Fgs.Bff.API/
...
RUN --mount=type=cache,target=/root/.nuget/packages \
    /usr/local/bin/restore-with-retry.sh Fgs.Bff.API.csproj && \
    dotnet build Fgs.Bff.API.csproj -c Release --no-restore
...
ENTRYPOINT ["dotnet", "run", "--no-build", "--no-launch-profile", "--project", "Fgs.Bff.API.csproj", "--configuration", "Release", "--urls", "http://+:5003"]
