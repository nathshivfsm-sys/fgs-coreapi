...
COPY src/InventoryService/Fgs.Inventory.API/Fgs.Inventory.API.csproj src/InventoryService/Fgs.Inventory.API/
COPY src/InventoryService/Fgs.Inventory.Application/Fgs.Inventory.Application.csproj src/InventoryService/Fgs.Inventory.Application/
...
RUN --mount=type=cache,target=/root/.nuget/packages \
    dotnet build Fgs.Inventory.API.csproj -c Release --no-restore
...
HEALTHCHECK --interval=30s --timeout=5s --start-period=90s --retries=5 \
    CMD curl -fsS http://localhost:5012/health || exit 1

ENTRYPOINT ["dotnet", "run", "--no-build", "--no-launch-profile", "--project", "Fgs.Inventory.API.csproj", "--configuration", "Release", "--urls", "http://+:5012"]
