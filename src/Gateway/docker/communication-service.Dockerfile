...
COPY src/CommunicationService/Fgs.Communication.API/Fgs.Communication.API.csproj src/CommunicationService/Fgs.Communication.API/
COPY src/CommunicationService/Fgs.Communication.Application/Fgs.Communication.Application.csproj src/CommunicationService/Fgs.Communication.Application/
...
RUN --mount=type=cache,target=/root/.nuget/packages \
    dotnet build Fgs.Communication.API.csproj -c Release --no-restore
...
HEALTHCHECK --interval=30s --timeout=5s --start-period=90s --retries=5 \
    CMD curl -fsS http://localhost:5017/health || exit 1

ENTRYPOINT ["dotnet", "run", "--no-build", "--no-launch-profile", "--project", "Fgs.Communication.API.csproj", "--configuration", "Release", "--urls", "http://+:5017"]
