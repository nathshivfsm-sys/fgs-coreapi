...
public sealed class FgsAssetAttributeOptionWriteService : IFgsAssetAttributeOptionWriteService
{
    ...
    private async Task<FgsAssetAttributeOption?> Find(long id, CancellationToken ct) => await _context.FgsAssetAttributeOptions.FirstOrDefaultIncludingInactiveAsync(x => x.Id == id, ct);
