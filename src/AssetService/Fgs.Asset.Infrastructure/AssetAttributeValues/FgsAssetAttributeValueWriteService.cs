...
public sealed class FgsAssetAttributeValueWriteService : IFgsAssetAttributeValueWriteService
{
    ...
    private async Task<FgsAssetAttributeValue?> Find(long id, CancellationToken ct) => await _context.FgsAssetAttributeValues.FirstOrDefaultIncludingInactiveAsync(x => x.Id == id, ct);
