...
public sealed class FgsAssetWarrantyWriteService : IFgsAssetWarrantyWriteService
{
    ...
    private async Task<FgsAssetWarranty?> Find(long id, CancellationToken ct) => await _context.FgsAssetWarranties.FirstOrDefaultIncludingInactiveAsync(x => x.Id == id, ct);
