...
public sealed class UpdateFgsInventorySerialCommandHandler(
    IFgsInventorySerialWriteService writeService,
    ICacheService cache,
    ITenantContextAccessor tenantContextAccessor,
    ILogger<UpdateFgsInventorySerialCommandHandler> logger)
    : IRequestHandler<UpdateFgsInventorySerialCommand, ApiResponse<FgsInventorySerialDetailDto>>
{
    public async Task<ApiResponse<FgsInventorySerialDetailDto>> Handle(
        UpdateFgsInventorySerialCommand request,
        CancellationToken cancellationToken)
    {
        var result = await writeService.UpdateAsync(request.Id, request.Dto, cancellationToken);
        logger.LogInformation("Updated inventory serial {Id}", result.Id);
