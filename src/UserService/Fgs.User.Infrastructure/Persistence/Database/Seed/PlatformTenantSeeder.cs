    ...
    ILogger<PlatformTenantSeeder> logger)
{
    public async Task SeedAsync(CancellationToken cancellationToken = default)
    {
        ...
        await dbContext.Database.ExecuteSqlRawAsync(
            $"""
            INSERT INTO {tenantTable} ("Id", "TenantCode", "Name", "FgsTenantStatusId", "IsActive", "CreatedOn")
            OVERRIDING SYSTEM VALUE
            SELECT 0, '{TenantScopeConstants.PlatformTenantCode}', 'Platform Global', 3, true, timezone('utc', now())
        ...
        await dbContext.Database.ExecuteSqlRawAsync(
            ...
            SELECT
                ...
                '{TenantScopeConstants.PlatformTenantCode}',
                'Platform Global',
                true,
                timezone('utc', now())
            WHERE NOT EXISTS (
                SELECT 1 FROM {companyTable}
