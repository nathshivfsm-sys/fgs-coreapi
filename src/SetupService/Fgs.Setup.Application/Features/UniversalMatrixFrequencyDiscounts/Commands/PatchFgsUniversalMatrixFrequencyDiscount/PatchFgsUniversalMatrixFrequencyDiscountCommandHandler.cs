...
public sealed class PatchFgsUniversalMatrixFrequencyDiscountCommandHandler(
    IFgsUniversalMatrixFrequencyDiscountWriteService writeService,
    ICacheService cache,
    ITenantContextAccessor tenantContextAccessor,
    ILogger<PatchFgsUniversalMatrixFrequencyDiscountCommandHandler> logger)
    : IRequestHandler<PatchFgsUniversalMatrixFrequencyDiscountCommand, ApiResponse<FgsUniversalMatrixFrequencyDiscountDetailDto>>
{
    public async Task<ApiResponse<FgsUniversalMatrixFrequencyDiscountDetailDto>> Handle(
        PatchFgsUniversalMatrixFrequencyDiscountCommand request,
        CancellationToken cancellationToken)
    {
        var result = await writeService.PatchAsync(request.Id, request.Dto, cancellationToken);
        logger.LogInformation("Patched universal matrix frequency discount {Id}", result.Id);
