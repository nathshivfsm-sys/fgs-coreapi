...
$entitiesRoot = Join-Path $infra 'Persistence'
...
foreach ($folder in $entityFolders) {
    ...
    if (Test-Path $source) {
        ...
        Write-Host "Already under Persistence: $folder"
    } else {
...
foreach ($child in $universalChildren) {
    ...
    $new = "Fgs.Setup.Infrastructure.Persistence.UniversalPricingMatrix.$child"
...
foreach ($folder in $entityFolders) {
    ...
    $new = "Fgs.Setup.Infrastructure.Persistence.$folder"
    foreach ($file in $csFiles) {
        $content = Get-Content $file.FullName -Raw
        if ($content -match [regex]::Escape($old)) {
            Set-Content -Path $file.FullName -Value ($content.Replace($old, $new)) -NoNewline
...
Write-Host 'Done reorganizing Infrastructure persistence folders.'
