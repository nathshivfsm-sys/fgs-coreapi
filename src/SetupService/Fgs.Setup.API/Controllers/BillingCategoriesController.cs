...
public sealed class BillingCategoriesController(IMediator mediator) : ControllerBase
{
    [HttpGet("{id:long}")]
    ...
    {
        var response = await mediator.Send(new DeleteBillingCategoryCommand(id), cancellationToken);
        return StatusCode(response.StatusCode, response);
    }
}
