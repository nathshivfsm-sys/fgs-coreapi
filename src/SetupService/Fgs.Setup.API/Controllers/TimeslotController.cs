...
[FgsVersionedRoute("timeslot")]
[Produces("application/json")]
public sealed class TimeslotController(IMediator mediator) : ControllerBase
{
    [HttpGet("{id:long}")]
    ...
    {
        var response = await mediator.Send(
            new LookupSetupTimeSlotsQuery(activeOnly, isMobileVisible, isCustomerPortalVisible),
            cancellationToken);
        return StatusCode(response.StatusCode, response);
    }
