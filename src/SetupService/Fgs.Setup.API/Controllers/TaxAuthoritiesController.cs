...
public sealed class TaxAuthoritiesController(IMediator mediator) : ControllerBase
{
    [HttpGet("{id:long}")]
    ...
    {
        var response = await mediator.Send(new DeleteFgsSetupTaxAuthorityCommand(id), cancellationToken);
        return StatusCode(response.StatusCode, response);
    }
}
