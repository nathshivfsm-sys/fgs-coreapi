...
using Fgs.Setup.Application.Features.SetupPaymentTerms.Queries.ListActiveSetupPaymentTerms;
...
public sealed class PaymentTermsController(IMediator mediator) : ControllerBase
{
    ...
    {
        var response = await mediator.Send(
            new ListActiveSetupPaymentTermsQuery(
                page,
                pageSize,
                sortBy,
                sortDirection,
    ...
    {
        var response = await mediator.Send(new DeleteFgsSetupPaymentTermCommand(id), cancellationToken);
        return StatusCode(response.StatusCode, response);
    }
}
