...
public sealed class ZonesController(IMediator mediator) : ControllerBase
{
    [HttpGet("{id:long}")]
    ...
    {
        var response = await mediator.Send(new DeleteFgsSetupZoneCommand(id), cancellationToken);
        return StatusCode(response.StatusCode, response);
    }
}
