...
public sealed class FgsUniversalPricingServiceWriteRepository : IFgsUniversalPricingServiceWriteRepository
{
    private readonly FgsSetupDbContext _context;
    private readonly IUnitOfWork _unitOfWork;
    private readonly SetupEntityAuditHelper _auditHelper;

    public FgsUniversalPricingServiceWriteRepository(
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            UniversalPricingServiceCode = NormalizeCode(dto.UniversalPricingServiceCode), DisplayOrder = dto.DisplayOrder
        };
    ...
    public async Task<FgsUniversalPricingServiceDetailDto> DeleteAsync(long id, CancellationToken cancellationToken = default)
    {
        ...
        var activeChildCount =
            await _context.FgsUniversalMatrixTiers.CountAsync(e => e.UniversalPricingServiceId == id && e.IsActive, cancellationToken)
            + await _context.FgsUniversalMatrixSizeTiers.CountAsync(e => e.UniversalPricingServiceId == id && e.IsActive, cancellationToken)
            + await _context.FgsUniversalMatrixItems.CountAsync(e => e.UniversalPricingServiceId == id && e.IsActive, cancellationToken)
            + await _context.FgsUniversalMatrixFrequencyDiscounts.CountAsync(e => e.UniversalPricingServiceId == id && e.IsActive, cancellationToken)
            + await _context.FgsUniversalMatrixOneTimeFees.CountAsync(e => e.UniversalPricingServiceId == id && e.IsActive, cancellationToken)
            + await _context.FgsUniversalMatrixAddOns.CountAsync(e => e.UniversalPricingServiceId == id && e.IsActive, cancellationToken);

        if (activeChildCount > 0)
        {
            throw new InvalidOperationException(
                $"Cannot delete universal pricing service '{entity.UniversalPricingServiceCode}' because it is referenced by {activeChildCount} active matrix record(s).");
        }
    ...
    private static FgsUniversalPricingServiceDetailDto MapToDetail(FgsUniversalPricingService entity) =>
        new(
            entity.Id,
            entity.UniversalPricingServiceCode,
            entity.DisplayOrder,
            entity.IsActive);
}
