...
internal sealed class FgsUniversalMatrixTierReadRepository : IFgsUniversalMatrixTierReadRepository
{
    private readonly ISetupReadConnectionFactory _connectionFactory;
    private readonly ITenantContextAccessor _tenantContextAccessor;

    public FgsUniversalMatrixTierReadRepository(
    ...
    public async Task<FgsUniversalMatrixTierDetailDto?> GetByIdAsync(long id, CancellationToken cancellationToken = default)
    {
        var (tenantId, companyId) = SetupTenantScopeResolver.ResolveRequired(_tenantContextAccessor);
        var sql = $"""
            SELECT {FgsUniversalMatrixTierSql.SelectDetailColumns}
            FROM {FgsUniversalMatrixTierSql.Table}
            WHERE "Id" = @Id
        ...
        var row = await connection.QueryFirstOrDefaultAsync<FgsUniversalMatrixTierDetailRow>(
            new CommandDefinition(sql, new { Id = id, TenantId = tenantId, CompanyId = companyId }, cancellationToken: cancellationToken));

        return row?.ToDto();
    }

    public async Task<PagedResult<FgsUniversalMatrixTierSummaryDto>> ListAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var orderBy = FgsUniversalMatrixTierSql.ResolveOrderBy(paging.SortBy, paging.SortDirection);

        var sql = $"""
            SELECT {FgsUniversalMatrixTierSql.SelectSummaryColumns}
            FROM {FgsUniversalMatrixTierSql.Table}
            WHERE {whereClause}
            {orderBy}
        ...
        var rows = (await multi.ReadAsync<FgsUniversalMatrixTierSummaryRow>()).ToList();
        var totalCount = await multi.ReadSingleAsync<int>();

        return new PagedResult<FgsUniversalMatrixTierSummaryDto>(
    ...
    public async Task<IReadOnlyList<FgsUniversalMatrixTierLookupDto>> LookupAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var parentFilter = universalPricingServiceId.HasValue
            ...
            : string.Empty;
        var sql = $"""
            SELECT {FgsUniversalMatrixTierSql.SelectLookupColumns}
            FROM {FgsUniversalMatrixTierSql.Table}
        ...
        await using var connection = await _connectionFactory.CreateOpenConnectionAsync(cancellationToken);
        var rows = await connection.QueryAsync<FgsUniversalMatrixTierLookupRow>(
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var sql = $"""
            SELECT EXISTS(
                SELECT 1
                FROM {FgsUniversalMatrixTierSql.ParentTable}
