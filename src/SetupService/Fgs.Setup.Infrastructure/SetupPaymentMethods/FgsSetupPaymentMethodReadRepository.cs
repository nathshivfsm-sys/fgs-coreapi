...
internal sealed class FgsSetupPaymentMethodReadRepository : IFgsSetupPaymentMethodReadRepository
{
    private readonly ISetupReadConnectionFactory _connectionFactory;
    private readonly ITenantContextAccessor _tenantContextAccessor;

    public FgsSetupPaymentMethodReadRepository(
    ...
    public async Task<FgsSetupPaymentMethodDetailDto?> GetByIdAsync(long id, CancellationToken cancellationToken = default)
    {
        ...
        var row = await connection.QueryFirstOrDefaultAsync<FgsSetupPaymentMethodDetailRow>(
            new CommandDefinition(sql, new { Id = id, TenantId = tenantId, CompanyId = companyId }, cancellationToken: cancellationToken));

        return row?.ToDto();
    }

    public async Task<PagedResult<FgsSetupPaymentMethodSummaryDto>> ListAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var orderBy = FgsSetupPaymentMethodSql.ResolveOrderBy(paging.SortBy, paging.SortDirection);

        var sql = $"""
            SELECT {FgsSetupPaymentMethodSql.SelectSummaryColumns}
            FROM {FgsSetupPaymentMethodSql.Table}
            WHERE {whereClause}
            {orderBy}
        ...
        var rows = (await multi.ReadAsync<FgsSetupPaymentMethodSummaryRow>()).ToList();
        var totalCount = await multi.ReadSingleAsync<int>();

        return new PagedResult<FgsSetupPaymentMethodSummaryDto>(
