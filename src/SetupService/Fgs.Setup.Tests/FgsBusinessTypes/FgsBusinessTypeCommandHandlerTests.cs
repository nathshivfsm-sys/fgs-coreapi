...
public sealed class FgsBusinessTypeCommandHandlerTests
{
    ...
    public async Task CreateHandler_CreatesActiveRecord()
    {
        ...
        cache.Verify(
            c => c.RemoveByPrefixAsync(
                CacheKeys.EntityPrefix(TenantId, CompanyId, "businesstype"),
    ...
    public async Task DeleteHandler_SoftDeletes()
    {
        ...
        var response = await deleteHandler.Handle(
            new DeleteFgsBusinessTypeCommand(created.Data!.Id),
            CancellationToken.None);
    ...
    private static FgsBusinessTypeWriteService CreateWriteService(FgsSetupDbContext context)
    {
        ...
        return new FgsBusinessTypeWriteService(context, unitOfWork, auditHelper);
    }
