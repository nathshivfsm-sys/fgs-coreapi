...
public sealed class FgsSetupDescriptionCommandHandlerTests
{
    ...
    public async Task CreateHandler_CreatesActiveRecord()
    {
        ...
        var handler = new CreateFgsSetupDescriptionCommandHandler(
            writeService,
            cache.Object,
            tenantAccessor,
            NullLogger<CreateFgsSetupDescriptionCommandHandler>.Instance);

        var response = await handler.Handle(
            new CreateFgsSetupDescriptionCommand(new FgsSetupDescriptionCreateDto("DescriptionTypeCode value", "ShortNote", "Body value", null, 1)),
    ...
    public async Task DeleteHandler_SoftDeletes()
    {
        ...
        var cache = new Mock<ICacheService>();
        var tenantAccessor = CreateTenantContextAccessor();
        var createHandler = new CreateFgsSetupDescriptionCommandHandler(
            writeService,
            cache.Object,
            tenantAccessor,
            NullLogger<CreateFgsSetupDescriptionCommandHandler>.Instance);
        var deleteHandler = new DeleteFgsSetupDescriptionCommandHandler(
            writeService,
            cache.Object,
            tenantAccessor,
            NullLogger<DeleteFgsSetupDescriptionCommandHandler>.Instance);
        ...
        var response = await deleteHandler.Handle(
            new DeleteFgsSetupDescriptionCommand(created.Data!.Id),
            CancellationToken.None);
    ...
    private static FgsSetupDescriptionWriteService CreateWriteService(FgsSetupDbContext context)
    {
        ...
        return new FgsSetupDescriptionWriteService(context, unitOfWork, auditHelper);
    }
