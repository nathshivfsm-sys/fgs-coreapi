...
using Fgs.Setup.Infrastructure.SetupTaxDetails;
...
public sealed class FgsSetupTaxDetailCommandHandlerTests
{
    ...
    public async Task CreateHandler_CreatesWithAuditFields()
    {
        ...
        var handler = new CreateFgsSetupTaxDetailCommandHandler(
            writeService,
            NullLogger<CreateFgsSetupTaxDetailCommandHandler>.Instance);

        var response = await handler.Handle(
            new CreateFgsSetupTaxDetailCommand(new FgsSetupTaxDetailCreateDto(1, 1, DateOnly.FromDateTime(DateTime.UtcNow), null, 10.5m, false)),
    ...
    public async Task DeleteHandler_SoftDeletes()
    {
        await using var context = await CreateContextAsync();
        var writeService = CreateWriteService(context);
        var createHandler = new CreateFgsSetupTaxDetailCommandHandler(
            writeService,
            NullLogger<CreateFgsSetupTaxDetailCommandHandler>.Instance);
        var deleteHandler = new DeleteFgsSetupTaxDetailCommandHandler(
            writeService,
            NullLogger<DeleteFgsSetupTaxDetailCommandHandler>.Instance);
        ...
        var response = await deleteHandler.Handle(
            new DeleteFgsSetupTaxDetailCommand(created.Data!.Id),
            CancellationToken.None);
    ...
    private static FgsSetupTaxDetailWriteService CreateWriteService(FgsSetupDbContext context)
    {
        ...
        return new FgsSetupTaxDetailWriteService(context, unitOfWork, auditHelper);
    }
