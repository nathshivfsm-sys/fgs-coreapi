...
using Fgs.Setup.Application.Features.JobCategories.Commands.DeleteJobCategory;
...
public sealed class JobCategoryCommandHandlerTests
{
    ...
    public async Task CreateHandler_CreatesActiveRecord()
    {
        ...
        var response = await handler.Handle(
            new CreateJobCategoryCommand(new JobCategoryCreateDto("TEST", "Name", 1)),
        ...
        cache.Verify(
            c => c.RemoveByPrefixAsync(
                CacheKeys.EntityPrefix(TenantId, CompanyId, "jobcategory"),
    ...
    public async Task DeleteHandler_SoftDeletes()
    {
        ...
        var cache = new Mock<ICacheService>();
        var tenantAccessor = CreateTenantContextAccessor();
        var createHandler = new CreateJobCategoryCommandHandler(
            writeService,
            cache.Object,
            tenantAccessor,
            NullLogger<CreateJobCategoryCommandHandler>.Instance);
        var deleteHandler = new DeleteJobCategoryCommandHandler(
            writeService,
            cache.Object,
            tenantAccessor,
            NullLogger<DeleteJobCategoryCommandHandler>.Instance);
        ...
        var response = await deleteHandler.Handle(
            new DeleteJobCategoryCommand(created.Data!.Id),
            CancellationToken.None);
    ...
    private static JobCategoryWriteService CreateWriteService(FgsSetupDbContext context)
    {
        ...
        return new JobCategoryWriteService(context, unitOfWork, auditHelper);
    }
