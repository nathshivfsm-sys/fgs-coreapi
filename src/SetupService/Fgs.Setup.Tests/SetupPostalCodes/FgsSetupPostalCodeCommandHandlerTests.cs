...
public sealed class FgsSetupPostalCodeCommandHandlerTests
{
    ...
    public async Task CreateHandler_CreatesActiveRecord()
    {
        ...
        var handler = new CreateFgsSetupPostalCodeCommandHandler(
            writeService,
            cache.Object,
            tenantAccessor,
        ...
        cache.Verify(
            c => c.RemoveByPrefixAsync(
                CacheKeys.EntityPrefix(TenantId, CompanyId, "postalcode"),
    ...
    public async Task DeleteHandler_SoftDeletes()
    {
        ...
        var cache = new Mock<ICacheService>();
        var tenantAccessor = CreateTenantContextAccessor();
        var createHandler = new CreateFgsSetupPostalCodeCommandHandler(
            writeService,
            cache.Object,
            tenantAccessor,
            NullLogger<CreateFgsSetupPostalCodeCommandHandler>.Instance);
        var deleteHandler = new DeleteFgsSetupPostalCodeCommandHandler(
            writeService,
            cache.Object,
            tenantAccessor,
        ...
        var response = await deleteHandler.Handle(
            new DeleteFgsSetupPostalCodeCommand(created.Data!.Id),
            CancellationToken.None);
    ...
    private static FgsSetupPostalCodeWriteService CreateWriteService(FgsSetupDbContext context)
    {
        ...
        return new FgsSetupPostalCodeWriteService(context, unitOfWork, auditHelper);
    }
