...
public sealed class LeadDisqualificationReasonCommandHandlerTests
{
    ...
    public async Task DeleteHandler_SoftDeletes()
    {
        ...
        var response = await deleteHandler.Handle(
            new DeleteLeadDisqualificationReasonCommand(created.Data!.Id),
            CancellationToken.None);
    ...
    private static LeadDisqualificationReasonWriteService CreateWriteService(FgsSetupDbContext context)
    {
        ...
        return new LeadDisqualificationReasonWriteService(context, unitOfWork, auditHelper);
    }
