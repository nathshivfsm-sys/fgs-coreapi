...
using Fgs.Setup.Application.Features.UniversalMatrixSizeTiers.Commands.DeleteFgsUniversalMatrixSizeTier;
...
public sealed class FgsUniversalMatrixSizeTierCommandHandlerTests
{
    ...
    public async Task CreateHandler_CreatesActiveRecord()
    {
        ...
        var handler = new CreateFgsUniversalMatrixSizeTierCommandHandler(
            writeService,
            cache.Object,
            tenantAccessor,
            NullLogger<CreateFgsUniversalMatrixSizeTierCommandHandler>.Instance);

        var response = await handler.Handle(
            new CreateFgsUniversalMatrixSizeTierCommand(new FgsUniversalMatrixSizeTierCreateDto(1, "Name", 10.5m, 5)),
        ...
        cache.Verify(
            c => c.RemoveByPrefixAsync(
                CacheKeys.EntityPrefix(TenantId, CompanyId, "universalmatrixsizetier"),
    ...
    public async Task DeleteHandler_SoftDeletes()
    {
        ...
        var cache = new Mock<ICacheService>();
        var tenantAccessor = CreateTenantContextAccessor();
        var createHandler = new CreateFgsUniversalMatrixSizeTierCommandHandler(
            writeService,
            cache.Object,
            tenantAccessor,
            NullLogger<CreateFgsUniversalMatrixSizeTierCommandHandler>.Instance);
        var deleteHandler = new DeleteFgsUniversalMatrixSizeTierCommandHandler(
            writeService,
            cache.Object,
            tenantAccessor,
            NullLogger<DeleteFgsUniversalMatrixSizeTierCommandHandler>.Instance);
        ...
        var response = await deleteHandler.Handle(
            new DeleteFgsUniversalMatrixSizeTierCommand(created.Data!.Id),
            CancellationToken.None);
    ...
    private static FgsUniversalMatrixSizeTierWriteRepository CreateWriteService(FgsSetupDbContext context)
    {
        ...
        return new FgsUniversalMatrixSizeTierWriteRepository(context, unitOfWork, auditHelper);
    }
