...
public sealed class FgsSetupTimeSlotCommandHandlerTests
{
    ...
    public async Task CreateHandler_CreatesActiveRecord()
    {
        ...
        var handler = new CreateFgsSetupTimeSlotCommandHandler(
            writeService,
            cache.Object,
            tenantAccessor,
            NullLogger<CreateFgsSetupTimeSlotCommandHandler>.Instance);

        var response = await handler.Handle(
            new CreateFgsSetupTimeSlotCommand(new FgsSetupTimeSlotCreateDto(null, "TEST", "Name value", TimeSpan.FromHours(8), TimeSpan.FromHours(17), null, null, true, true)),
        ...
        cache.Verify(
            c => c.RemoveByPrefixAsync(
                CacheKeys.EntityPrefix(TenantId, CompanyId, "timeslots"),
    ...
    public async Task DeleteHandler_SoftDeletes()
    {
        ...
        var cache = new Mock<ICacheService>();
        var tenantAccessor = CreateTenantContextAccessor();
        var createHandler = new CreateFgsSetupTimeSlotCommandHandler(
            writeService,
            cache.Object,
            tenantAccessor,
            NullLogger<CreateFgsSetupTimeSlotCommandHandler>.Instance);
        var deleteHandler = new DeleteFgsSetupTimeSlotCommandHandler(
            writeService,
            cache.Object,
            tenantAccessor,
            NullLogger<DeleteFgsSetupTimeSlotCommandHandler>.Instance);
        ...
        var response = await deleteHandler.Handle(
            new DeleteFgsSetupTimeSlotCommand(created.Data!.Id),
            CancellationToken.None);
    ...
    private static FgsSetupTimeSlotWriteService CreateWriteService(FgsSetupDbContext context)
    {
        ...
        return new FgsSetupTimeSlotWriteService(context, unitOfWork, auditHelper);
    }
