...
public sealed class FgsSetupTechSkillLevelCommandHandlerTests
{
    ...
    public async Task CreateHandler_CreatesActiveRecord()
    {
        ...
        var handler = new CreateFgsSetupTechSkillLevelCommandHandler(
            writeService,
            cache.Object,
            tenantAccessor,
            NullLogger<CreateFgsSetupTechSkillLevelCommandHandler>.Instance);

        var response = await handler.Handle(
            new CreateFgsSetupTechSkillLevelCommand(new FgsSetupTechSkillLevelCreateDto("TEST", "Name value", "Description value", 60)),
        ...
        cache.Verify(
            c => c.RemoveByPrefixAsync(
                CacheKeys.EntityPrefix(TenantId, CompanyId, "techskilllevels"),
    ...
    public async Task DeleteHandler_SoftDeletes()
    {
        ...
        var cache = new Mock<ICacheService>();
        var tenantAccessor = CreateTenantContextAccessor();
        var createHandler = new CreateFgsSetupTechSkillLevelCommandHandler(
            writeService,
            cache.Object,
            tenantAccessor,
            NullLogger<CreateFgsSetupTechSkillLevelCommandHandler>.Instance);
        var deleteHandler = new DeleteFgsSetupTechSkillLevelCommandHandler(
            writeService,
            cache.Object,
            tenantAccessor,
            NullLogger<DeleteFgsSetupTechSkillLevelCommandHandler>.Instance);
        ...
        var response = await deleteHandler.Handle(
            new DeleteFgsSetupTechSkillLevelCommand(created.Data!.Id),
            CancellationToken.None);
    ...
    private static FgsSetupTechSkillLevelWriteService CreateWriteService(FgsSetupDbContext context)
    {
        ...
        return new FgsSetupTechSkillLevelWriteService(context, unitOfWork, auditHelper);
    }
