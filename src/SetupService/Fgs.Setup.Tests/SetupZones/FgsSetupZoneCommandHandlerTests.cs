...
using Fgs.Setup.Application.Features.SetupZones.Commands.DeleteFgsSetupZone;
...
public sealed class FgsSetupZoneCommandHandlerTests
{
    ...
    public async Task CreateHandler_CreatesActiveRecord()
    {
        ...
        var handler = new CreateFgsSetupZoneCommandHandler(
            writeService,
            cache.Object,
            tenantAccessor,
            NullLogger<CreateFgsSetupZoneCommandHandler>.Instance);

        var response = await handler.Handle(
            new CreateFgsSetupZoneCommand(new FgsSetupZoneCreateDto("TEST", "Name value", "Description value")),
        ...
        cache.Verify(
            c => c.RemoveByPrefixAsync(
                CacheKeys.EntityPrefix(TenantId, CompanyId, "zones"),
    ...
    public async Task DeleteHandler_SoftDeletes()
    {
        ...
        var cache = new Mock<ICacheService>();
        var tenantAccessor = CreateTenantContextAccessor();
        var createHandler = new CreateFgsSetupZoneCommandHandler(
            writeService,
            cache.Object,
            tenantAccessor,
            NullLogger<CreateFgsSetupZoneCommandHandler>.Instance);
        var deleteHandler = new DeleteFgsSetupZoneCommandHandler(
            writeService,
            cache.Object,
            tenantAccessor,
            NullLogger<DeleteFgsSetupZoneCommandHandler>.Instance);
        ...
        var response = await deleteHandler.Handle(
            new DeleteFgsSetupZoneCommand(created.Data!.Id),
            CancellationToken.None);
    ...
    private static FgsSetupZoneWriteService CreateWriteService(FgsSetupDbContext context)
    {
        ...
        return new FgsSetupZoneWriteService(context, unitOfWork, auditHelper);
    }
