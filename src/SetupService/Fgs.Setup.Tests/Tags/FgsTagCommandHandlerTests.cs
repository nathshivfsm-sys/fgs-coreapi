...
public sealed class FgsTagCommandHandlerTests
{
    ...
    public async Task CreateHandler_CreatesActiveRecord()
    {
        ...
        var handler = new CreateFgsTagCommandHandler(
            writeService,
            cache.Object,
            tenantAccessor,
            NullLogger<CreateFgsTagCommandHandler>.Instance);

        var response = await handler.Handle(
            new CreateFgsTagCommand(new FgsTagCreateDto("TEST", "Name", "Description value", "BackgroundColor", "TextColor", null)),
    ...
    public async Task DeleteHandler_SoftDeletes()
    {
        ...
        var cache = new Mock<ICacheService>();
        var tenantAccessor = CreateTenantContextAccessor();
        var createHandler = new CreateFgsTagCommandHandler(
            writeService,
            cache.Object,
            tenantAccessor,
            NullLogger<CreateFgsTagCommandHandler>.Instance);
        var deleteHandler = new DeleteFgsTagCommandHandler(
            writeService,
            cache.Object,
            tenantAccessor,
            NullLogger<DeleteFgsTagCommandHandler>.Instance);
        ...
        var response = await deleteHandler.Handle(
            new DeleteFgsTagCommand(created.Data!.Id),
            CancellationToken.None);
    ...
    private static FgsTagWriteService CreateWriteService(FgsSetupDbContext context)
    {
        ...
        return new FgsTagWriteService(context, unitOfWork, auditHelper);
    }
