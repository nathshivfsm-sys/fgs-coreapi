...
public sealed class BillingCategoryQueryHandlerTests
{
    ...
    {
        var detail = new BillingCategoryDetailDto(1, 10, 20, "TEST", "BillingCategoryName value", "Description value", 1, false, false, true, true, DateTimeOffset.UtcNow, "seed", null, null);

        var readRepository = new Mock<IBillingCategoryReadRepository>();
        readRepository.Setup(r => r.GetByIdAsync(1, It.IsAny<CancellationToken>())).ReturnsAsync(detail);

        var handler = new GetBillingCategoryByIdQueryHandler(readRepository.Object);
