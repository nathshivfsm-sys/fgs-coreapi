...
public sealed class LeadStatusQueryHandlerTests
{
    ...
    {
        var detail = new LeadStatusDetailDto(1, 10, 20, "TEST", "StatusName value", "Description value", 1, false, true, DateTimeOffset.UtcNow, "seed", null, null);

        var readRepository = new Mock<ILeadStatusReadRepository>();
        readRepository.Setup(r => r.GetByIdAsync(1, It.IsAny<CancellationToken>())).ReturnsAsync(detail);

        var handler = new GetLeadStatusByIdQueryHandler(readRepository.Object);
