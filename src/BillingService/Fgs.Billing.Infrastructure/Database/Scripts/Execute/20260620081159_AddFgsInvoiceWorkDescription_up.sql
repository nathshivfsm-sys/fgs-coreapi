...
DO $EF$
BEGIN
    IF NOT EXISTS(SELECT 1 FROM billing."__EFMigrationsHistory" WHERE "MigrationId" = '20260620081159_AddFgsInvoiceWorkDescription') THEN
    CREATE TABLE billing."FgsInvoiceWorkDescription" (
        "TenantId" bigint NOT NULL,
        "CompanyId" bigint NOT NULL,
        "Id" bigint GENERATED ALWAYS AS IDENTITY,
        "InvoiceId" bigint NOT NULL,
        "ServiceDate" date NOT NULL,
        "TechCode" character varying(50),
        ...
        "UpdatedOn" timestamp,
        "UpdatedBy" bigint,
        CONSTRAINT "PK_FgsInvoiceWorkDescription" PRIMARY KEY ("Id"),
        CONSTRAINT "FK_FgsInvoiceWorkDescription_FgsInvoice" FOREIGN KEY ("InvoiceId") REFERENCES billing."FgsInvoice" ("Id") ON DELETE CASCADE,
        CONSTRAINT "FK_FgsInvoiceWorkDescription_TenantCompany" FOREIGN KEY ("TenantId", "CompanyId") REFERENCES billing."FgsTenantCompanyCache" ("TenantId", "CompanyId") ON DELETE RESTRICT
    );
    COMMENT ON TABLE billing."FgsInvoiceWorkDescription" IS 'Stores technician and office-entered work descriptions associated with an invoice. Supports multiple work performed entries for an invoice.';
    COMMENT ON COLUMN billing."FgsInvoiceWorkDescription"."InvoiceId" IS 'Parent invoice identifier.';
    COMMENT ON COLUMN billing."FgsInvoiceWorkDescription"."ServiceDate" IS 'Service date for the work performed entry.';
    COMMENT ON COLUMN billing."FgsInvoiceWorkDescription"."TechCode" IS 'Technician code associated with the work performed.';
    COMMENT ON COLUMN billing."FgsInvoiceWorkDescription"."UserName" IS 'User who entered the work description.';
    COMMENT ON COLUMN billing."FgsInvoiceWorkDescription"."WorkDescription" IS 'Work performed description.';
    COMMENT ON COLUMN billing."FgsInvoiceWorkDescription"."IsCustomerVisible" IS 'Indicates whether the work description is visible to the customer.';
    END IF;
END $EF$;
...
BEGIN
    IF NOT EXISTS(SELECT 1 FROM billing."__EFMigrationsHistory" WHERE "MigrationId" = '20260620081159_AddFgsInvoiceWorkDescription') THEN
    CREATE INDEX "IX_FgsInvoiceWorkDescription_InvoiceId" ON billing."FgsInvoiceWorkDescription" ("TenantId", "CompanyId", "InvoiceId");
    END IF;
END $EF$;
...
BEGIN
    IF NOT EXISTS(SELECT 1 FROM billing."__EFMigrationsHistory" WHERE "MigrationId" = '20260620081159_AddFgsInvoiceWorkDescription') THEN
    CREATE INDEX "IX_FgsInvoiceWorkDescription_IsCustomerVisible" ON billing."FgsInvoiceWorkDescription" ("TenantId", "CompanyId", "IsCustomerVisible");
    END IF;
END $EF$;
...
BEGIN
    IF NOT EXISTS(SELECT 1 FROM billing."__EFMigrationsHistory" WHERE "MigrationId" = '20260620081159_AddFgsInvoiceWorkDescription') THEN
    CREATE INDEX "IX_FgsInvoiceWorkDescription_ServiceDate" ON billing."FgsInvoiceWorkDescription" ("TenantId", "CompanyId", "ServiceDate");
    END IF;
END $EF$;
...
BEGIN
    IF NOT EXISTS(SELECT 1 FROM billing."__EFMigrationsHistory" WHERE "MigrationId" = '20260620081159_AddFgsInvoiceWorkDescription') THEN
    CREATE INDEX "IX_FgsInvoiceWorkDescription_TechCode" ON billing."FgsInvoiceWorkDescription" ("TenantId", "CompanyId", "TechCode");
    END IF;
END $EF$;
...
BEGIN
    IF NOT EXISTS(SELECT 1 FROM billing."__EFMigrationsHistory" WHERE "MigrationId" = '20260620081159_AddFgsInvoiceWorkDescription') THEN
    CREATE INDEX "IX_FgsInvoiceWorkDescription_TenantCompany" ON billing."FgsInvoiceWorkDescription" ("TenantId", "CompanyId");
    END IF;
END $EF$;
...
BEGIN
    IF NOT EXISTS(SELECT 1 FROM billing."__EFMigrationsHistory" WHERE "MigrationId" = '20260620081159_AddFgsInvoiceWorkDescription') THEN
    CREATE UNIQUE INDEX "UX_FgsInvoiceWorkDescription_Invoice_TechCode_ServiceDate" ON billing."FgsInvoiceWorkDescription" ("TenantId", "CompanyId", "InvoiceId", "TechCode", "ServiceDate") WHERE "TechCode" IS NOT NULL;
    END IF;
END $EF$;
...
BEGIN
    IF NOT EXISTS(SELECT 1 FROM billing."__EFMigrationsHistory" WHERE "MigrationId" = '20260620081159_AddFgsInvoiceWorkDescription') THEN
    INSERT INTO billing."__EFMigrationsHistory" ("MigrationId", "ProductVersion")
    VALUES ('20260620081159_AddFgsInvoiceWorkDescription', '10.0.8');
    END IF;
END $EF$;
COMMIT;
