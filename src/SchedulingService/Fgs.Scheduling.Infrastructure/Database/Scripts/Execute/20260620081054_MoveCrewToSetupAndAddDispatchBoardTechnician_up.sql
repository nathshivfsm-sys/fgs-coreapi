...
DO $EF$
BEGIN
    IF NOT EXISTS(SELECT 1 FROM dispatch."__EFMigrationsHistory" WHERE "MigrationId" = '20260620081054_MoveCrewToSetupAndAddDispatchBoardTechnician') THEN
    ALTER TABLE dispatch."FgsAppointment" DROP CONSTRAINT "FK_FgsAppointment_Crew";
    END IF;
END $EF$;
...
BEGIN
    IF NOT EXISTS(SELECT 1 FROM dispatch."__EFMigrationsHistory" WHERE "MigrationId" = '20260620081054_MoveCrewToSetupAndAddDispatchBoardTechnician') THEN
    ALTER TABLE dispatch."FgsAppointmentAssignment" DROP CONSTRAINT "FK_FgsAppointmentAssignment_Crew";
    END IF;
END $EF$;
...
BEGIN
    IF NOT EXISTS(SELECT 1 FROM dispatch."__EFMigrationsHistory" WHERE "MigrationId" = '20260620081054_MoveCrewToSetupAndAddDispatchBoardTechnician') THEN
    DROP TABLE dispatch."FgsCrewMember";
    END IF;
END $EF$;
...
BEGIN
    IF NOT EXISTS(SELECT 1 FROM dispatch."__EFMigrationsHistory" WHERE "MigrationId" = '20260620081054_MoveCrewToSetupAndAddDispatchBoardTechnician') THEN
    COMMENT ON COLUMN dispatch."FgsAppointmentAssignment"."CrewId" IS 'Crew assignment snapshot at the time of scheduling. References setup service; no FK by design.';
    END IF;
END $EF$;
...
BEGIN
    IF NOT EXISTS(SELECT 1 FROM dispatch."__EFMigrationsHistory" WHERE "MigrationId" = '20260620081054_MoveCrewToSetupAndAddDispatchBoardTechnician') THEN
    COMMENT ON COLUMN dispatch."FgsAppointment"."CrewId" IS 'Scheduled crew assigned to the appointment. References setup service; no FK by design.';
    END IF;
END $EF$;
...
BEGIN
    IF NOT EXISTS(SELECT 1 FROM dispatch."__EFMigrationsHistory" WHERE "MigrationId" = '20260620081054_MoveCrewToSetupAndAddDispatchBoardTechnician') THEN
    CREATE TABLE dispatch."FgsDispatchBoardTechnician" (
        "TenantId" bigint NOT NULL,
        "CompanyId" bigint NOT NULL,
        "Id" bigint GENERATED ALWAYS AS IDENTITY,
        "ServiceDate" date NOT NULL,
        "TechnicianProfileId" bigint NOT NULL,
        "TechCode" character varying(25) NOT NULL,
        "TechName" character varying(200) NOT NULL,
        "CrewId" bigint,
        "CrewCode" character varying(25),
        "CrewName" character varying(100),
        "DispatchBoardStatusId" smallint NOT NULL DEFAULT 0,
        "IsWorking" boolean NOT NULL DEFAULT TRUE,
        "CreatedOn" timestamptz NOT NULL DEFAULT (now()),
        "CreatedBy" character varying(100),
        "UpdatedOn" timestamptz,
        "UpdatedBy" character varying(100),
        CONSTRAINT "PK_FgsDispatchBoardTechnician" PRIMARY KEY ("Id"),
        CONSTRAINT "CK_FgsDispatchBoardTechnician_Status" CHECK ("DispatchBoardStatusId" IN (0, 1, 2, 3, 4, 5, 6)),
        CONSTRAINT "FK_FgsDispatchBoardTechnician_FgsTenantCompanyCache_TenantId_CompanyId" FOREIGN KEY ("TenantId", "CompanyId") REFERENCES dispatch."FgsTenantCompanyCache" ("TenantId", "CompanyId") ON DELETE RESTRICT
    );
    COMMENT ON TABLE dispatch."FgsDispatchBoardTechnician" IS 'Stores daily dispatch board technician projections used for scheduling and dispatching.';
    COMMENT ON COLUMN dispatch."FgsDispatchBoardTechnician"."TenantId" IS 'Tenant identifier.';
    COMMENT ON COLUMN dispatch."FgsDispatchBoardTechnician"."CompanyId" IS 'Company identifier.';
    COMMENT ON COLUMN dispatch."FgsDispatchBoardTechnician"."Id" IS 'Primary key.';
    COMMENT ON COLUMN dispatch."FgsDispatchBoardTechnician"."ServiceDate" IS 'Service date represented on the dispatch board.';
    COMMENT ON COLUMN dispatch."FgsDispatchBoardTechnician"."TechnicianProfileId" IS 'Reference to setup.FgsEmployeeTechnicianProfile. Stored without cross-domain foreign key.';
    COMMENT ON COLUMN dispatch."FgsDispatchBoardTechnician"."TechCode" IS 'Technician code snapshot used by dispatch board displays.';
    COMMENT ON COLUMN dispatch."FgsDispatchBoardTechnician"."TechName" IS 'Technician name snapshot used by dispatch board displays.';
    COMMENT ON COLUMN dispatch."FgsDispatchBoardTechnician"."CrewId" IS 'Daily crew assignment identifier. May be overridden for a specific service date.';
    COMMENT ON COLUMN dispatch."FgsDispatchBoardTechnician"."CrewCode" IS 'Crew code snapshot used for dispatch board grouping.';
    COMMENT ON COLUMN dispatch."FgsDispatchBoardTechnician"."CrewName" IS 'Crew name snapshot used for dispatch board grouping.';
    COMMENT ON COLUMN dispatch."FgsDispatchBoardTechnician"."DispatchBoardStatusId" IS 'Dispatch board status. 0=Available, 1=Assigned, 2=Dispatched, 3=Arrived, 4=Waiting, 5=Completed, 6=Off Duty.';
    COMMENT ON COLUMN dispatch."FgsDispatchBoardTechnician"."IsWorking" IS 'Indicates whether the technician should appear on the dispatch board for the service date.';
    COMMENT ON COLUMN dispatch."FgsDispatchBoardTechnician"."CreatedOn" IS 'Date and time the record was created.';
    COMMENT ON COLUMN dispatch."FgsDispatchBoardTechnician"."CreatedBy" IS 'User who created the record.';
    COMMENT ON COLUMN dispatch."FgsDispatchBoardTechnician"."UpdatedOn" IS 'Date and time the record was last updated.';
    COMMENT ON COLUMN dispatch."FgsDispatchBoardTechnician"."UpdatedBy" IS 'User who last updated the record.';
    END IF;
END $EF$;
...
BEGIN
    IF NOT EXISTS(SELECT 1 FROM dispatch."__EFMigrationsHistory" WHERE "MigrationId" = '20260620081054_MoveCrewToSetupAndAddDispatchBoardTechnician') THEN
    CREATE INDEX "IX_FgsDispatchBoardTechnician_Crew" ON dispatch."FgsDispatchBoardTechnician" ("TenantId", "CompanyId", "ServiceDate", "CrewId");
    END IF;
END $EF$;
...
BEGIN
    IF NOT EXISTS(SELECT 1 FROM dispatch."__EFMigrationsHistory" WHERE "MigrationId" = '20260620081054_MoveCrewToSetupAndAddDispatchBoardTechnician') THEN
    CREATE INDEX "IX_FgsDispatchBoardTechnician_ServiceDate" ON dispatch."FgsDispatchBoardTechnician" ("TenantId", "CompanyId", "ServiceDate");
    END IF;
END $EF$;
...
BEGIN
    IF NOT EXISTS(SELECT 1 FROM dispatch."__EFMigrationsHistory" WHERE "MigrationId" = '20260620081054_MoveCrewToSetupAndAddDispatchBoardTechnician') THEN
    CREATE INDEX "IX_FgsDispatchBoardTechnician_TenantCompany" ON dispatch."FgsDispatchBoardTechnician" ("TenantId", "CompanyId");
    END IF;
END $EF$;
...
BEGIN
    IF NOT EXISTS(SELECT 1 FROM dispatch."__EFMigrationsHistory" WHERE "MigrationId" = '20260620081054_MoveCrewToSetupAndAddDispatchBoardTechnician') THEN
    CREATE UNIQUE INDEX "UX_FgsDispatchBoardTechnician_ServiceDate_TechnicianProfileId" ON dispatch."FgsDispatchBoardTechnician" ("TenantId", "CompanyId", "ServiceDate", "TechnicianProfileId");
    END IF;
END $EF$;
...
BEGIN
    IF NOT EXISTS(SELECT 1 FROM dispatch."__EFMigrationsHistory" WHERE "MigrationId" = '20260620081054_MoveCrewToSetupAndAddDispatchBoardTechnician') THEN
    INSERT INTO dispatch."__EFMigrationsHistory" ("MigrationId", "ProductVersion")
    VALUES ('20260620081054_MoveCrewToSetupAndAddDispatchBoardTechnician', '10.0.8');
    END IF;
END $EF$;
COMMIT;
